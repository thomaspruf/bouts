...
bcounts=function (data, name = "exm", thresh = 25, min_torpid = NULL, 
          min_wake = NULL, t_res = 5) 
{
  hib = data
  fac = mean(diff(hib$Hours), na.rm = T) * 60/t_res
  nh = length(hib$Hours)
  ...
  if (!is.null(min_torpid)) {
    ntorp = floor(min_torpid/delta)
    for (i in 1:(n - ntorp - 1)) {
      if (sum(torpor[i:(i + ntorp)]) == 0) 
        break
    }
    wsa = i
  }
  ...
  hibstart = dtm[wsa]
  if (!is.null(min_wake)) {
    nwake = floor(min_wake/delta)
    for (i in wsa:(n - nwake - 1)) {
      if (sum(torpor[i:(i + nwake)]) == nwake) 
        break
    }
    if (i == (n - nwake - 1)) 
      wse = n - 1
  ...
  plot(Temp ~ dtm, type = "o", pch = ".", ylim = c(-5, 40), 
       col = "grey40", xlab = "Hours", ylab = "Body Temperature C", 
       cex.lab = 1)
  ...
  change = diff(torpor)
  ar_actv <- tme_at_tb_max <- ar_len <- ar_onset <- tb_max <- rep(0, 
                                                                  ar)
  ar_no = 0
  ar_start = wsa
  for (i in wsa:wse) {
    if (change[i] == 1) {
      ...
      points(dtm[i], Temp[i], col = "red", bg = "red", 
             pch = 24)
    }
    if (change[i] == -1) {
      ar_end = i
  ...
  to_actv <- to_len <- to_onset <- tb_min <- tme_at_tb_min <- rep(0, 
                                                                  ar)
  to_no = 0
  to_start = 0
  for (i in wsa:(wse - 1)) {
  ...
  results = data.frame(rep(sname, nb), index, to_onset, to_len, 
                       tb_min, to_actv, ar_onset, ar_len, tb_max, ar_actv)
  ...
  res = rbind(results, c("Mean", "--", "--", mto, mtomin, mtoactv, 
                         "--", mar, mtomax, maractv))
  res = rbind(res, c("Sum ", "--", "--", sto, "--", "--", "--", 
                     sar, "--", "--"))
  print(res)
  invisible(res)
}